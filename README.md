...
- **매장 목록**: 검색 및 필터링 기능
- **매장 상세**: 재고 현황 및 상품 목록
- **QR 스캔**: 실시간 카메라 스트림 + QR 감지
- **재고 관리**: 탭별 재고 현황 관리
- **설정**: 앱 정보 및 사용자 설정

### 백엔드 (Node.js + Socket.io)
- **실시간 QR 스캔**: Socket.io를 통한 실시간 통신
- **카메라 스트림 처리**: WebRTC + 실시간 프레임 전송
- **세션 관리**: 스캔 세션 추적 및 관리
- **데이터 저장**: MongoDB를 통한 영구 저장
- **REST API**: 매장, 재고, 세션 데이터 관리
...
# 환경 변수에서 MongoDB URI 설정
MONGODB_URI=mongodb://localhost:27017/3m-qr-scanner
```
...
// Socket.io로 서버에 연결
socket.emit('start-camera', { storeId, userId });
```
...
setInterval(() => {
  ...
  socket.emit('frame-data', { frame: frameData, timestamp });
}, 100);
```
...
```javascript
// QR 감지 시 서버로 데이터 전송
socket.emit('qr-detected', {
  qrData: '3M-ADH-001|상품명|카테고리|가격|재고',
  timestamp: Date.now()
});
```
...
socket.emit('qr-processed', {
  productCode, productName, category, price, stock
});
```
